...
pdf_document_get_info (PpsDocument *document)
{
	...
	info->contains_js = poppler_document_has_javascript (PDF_DOCUMENT (document)->document) ?
	                    PPS_DOCUMENT_CONTAINS_JS_YES : PPS_DOCUMENT_CONTAINS_JS_NO;
		     ...
		     PopplerAction *action)
{
	...
	if (unimplemented_action) {
		g_warning ("Unimplemented action: %s, please post a bug report "
			   "in Papers issue tracker (https://gitlab.gnome.org/GNOME/papers/issues) "
...
{
	return  (PpsFileExporterCapabilities) (
		PPS_FILE_EXPORTER_CAN_PAGE_SET |
