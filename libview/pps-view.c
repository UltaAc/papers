...
static void
pps_view_create_annotation (PpsView *view)
{
	PpsViewPrivate *priv = GET_PRIVATE (view);
	PpsPoint         start;
	PpsPoint         end;
	...
	pps_view_create_annotation_real (view, annot_page, start, end);
}
...
pps_view_scroll_event (GtkEventControllerScroll *self, gdouble dx, gdouble dy, GtkWidget *widget)
{
	...
	fit_width = pps_view_page_fits (view, GTK_ORIENTATION_HORIZONTAL);
	fit_height = pps_view_page_fits (view, GTK_ORIENTATION_VERTICAL);
			    ...
			    gpointer		 user_data)
{
	...
	if (priv->adding_annot_info.adding_annot && !priv->adding_annot_info.annot) {
		...
		pps_view_create_annotation (view);
...
pps_view_class_init (PpsViewClass *class)
{
	...
	signals[SIGNAL_ANNOT_CANCEL_ADD] = g_signal_new ("annot-cancel-add",
                         G_TYPE_FROM_CLASS (object_class),
                         G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                         G_STRUCT_OFFSET (PpsViewClass, annot_cancel_add),
                         NULL, NULL,
			 g_cclosure_marshal_VOID__VOID,
                         G_TYPE_NONE, 0,
                         G_TYPE_NONE);
