...
#define G_LOG_DOMAIN "PapersDaemon"
#include <glib.h>
...
#define PPS_TYPE_DAEMON_APPLICATION              (pps_daemon_application_get_type ())
#define PPS_DAEMON_APPLICATION(object)           (G_TYPE_CHECK_INSTANCE_CAST ((object), PPS_TYPE_DAEMON_APPLICATION, PpsDaemonApplication))
#define PPS_DAEMON_APPLICATION_CLASS(klass)      (G_TYPE_CHECK_CLASS_CAST ((klass), PPS_TYPE_DAEMON_APPLICATION, PpsDaemonApplicationClass))
#define PPS_IS_DAEMON_APPLICATION(object)        (G_TYPE_CHECK_INSTANCE_TYPE ((object), PPS_TYPE_DAEMON_APPLICATION))
#define PPS_IS_DAEMON_APPLICATION_CLASS(klass)   (G_TYPE_CHECK_CLASS_TYPE ((klass), PPS_TYPE_DAEMON_APPLICATION))
#define PPS_DAEMON_APPLICATION_GET_CLASS(obj)    (G_TYPE_INSTANCE_GET_CLASS ((obj), PPS_TYPE_DAEMON_APPLICATION, PpsDaemonApplicationClass))

typedef struct _PpsDaemonApplication        PpsDaemonApplication;
typedef struct _PpsDaemonApplicationClass   PpsDaemonApplicationClass;

struct _PpsDaemonApplicationClass {
        GApplicationClass parent_class;
};

struct _PpsDaemonApplication
{
...
static GType pps_daemon_application_get_type (void);
G_DEFINE_TYPE (PpsDaemonApplication, pps_daemon_application, G_TYPE_APPLICATION)
...
static PpsDoc *
pps_daemon_application_find_doc (PpsDaemonApplication *application,
                                const gchar *uri)
{
	GList *l;

	for (l = application->docs; l != NULL; l = l->next) {
		PpsDoc *doc = (PpsDoc *)l->data;
...
spawn_papers (const gchar *uri)
{
	...
	if (!retval) {
		g_printerr ("Error spawning papers for uri %s: %s\n", uri, error->message);
		g_error_free (error);
	}
	g_free (argv[0]);
...
name_vanished_cb (GDBusConnection *connection,
                  ...
                  gpointer         user_data)
{
        PpsDaemonApplication *application = PPS_DAEMON_APPLICATION (user_data);
	GList *l;
...
static void
process_pending_invocations (PpsDaemonApplication *application,
                             const gchar *uri,
		    ...
		    gpointer         user_data)
{
	...
	g_variant_get (parameters, "(&s)", &uri);
        doc = pps_daemon_application_find_doc (application, uri);
...
static gboolean
handle_register_document_cb (PpsDaemon              *object,
                             GDBusMethodInvocation *invocation,
                             const gchar           *uri,
                             PpsDaemonApplication   *application)
{
        ...
        if (doc != NULL) {
                ...
                pps_daemon_complete_register_document (object, invocation, doc->dbus_name);
        ...
        doc->loaded_id = g_dbus_connection_signal_subscribe (connection,
                                                             doc->dbus_name,
                                                             PPS_DBUS_WINDOW_INTERFACE_NAME,
                                                             "DocumentLoaded",
                                                             NULL,
                                                             NULL,
                                                             0,
        ...
        pps_daemon_complete_register_document (object, invocation, "");
...
static gboolean
handle_unregister_document_cb (PpsDaemon              *object,
                               GDBusMethodInvocation *invocation,
                               const gchar           *uri,
                               PpsDaemonApplication   *application)
{
        ...
        pps_daemon_complete_unregister_document (object, invocation);
...
static gboolean
handle_find_document_cb (PpsDaemon              *object,
                         GDBusMethodInvocation *invocation,
                         const gchar           *uri,
                         gboolean               spawn,
                         PpsDaemonApplication   *application)
{
        ...
        doc = pps_daemon_application_find_doc (application, uri);
        if (doc != NULL) {
                pps_daemon_complete_find_document (object, invocation, doc->dbus_name);
        ...
        if (spawn) {
                ...
                if (uri_invocations == NULL) {
                        /* Only spawn once. */
                        ret_val = spawn_papers (uri);
                }

                if (ret_val) {
                        /* Only defer DBUS answer if papers was succesfully spawned */
        ...
        pps_daemon_complete_find_document (object, invocation, "");
...
static gboolean
pps_daemon_application_dbus_register (GApplication    *gapplication,
                                     GDBusConnection *connection,
                                     const gchar     *object_path,
                                     GError         **error)
{
        PpsDaemonApplication *application = PPS_DAEMON_APPLICATION (gapplication);
        PpsDaemon *skeleton;

        if (!G_APPLICATION_CLASS (pps_daemon_application_parent_class)->dbus_register (gapplication,
                                                                                      connection,
                                                                                      object_path,
                                                                                      error))
                return FALSE;

        skeleton = pps_daemon_skeleton_new ();
        if (!g_dbus_interface_skeleton_export (G_DBUS_INTERFACE_SKELETON (skeleton),
                                               connection,
                                               PPS_DBUS_DAEMON_OBJECT_PATH,
                                               error)) {
...
static void
pps_daemon_application_dbus_unregister (GApplication    *gapplication,
                                       GDBusConnection *connection,
                                       const gchar     *object_path)
{
        PpsDaemonApplication *application = PPS_DAEMON_APPLICATION (gapplication);
        ...
        G_APPLICATION_CLASS (pps_daemon_application_parent_class)->dbus_unregister (gapplication,
                                                                                   connection,
                                                                                   object_path);
}

static void
pps_daemon_application_init (PpsDaemonApplication *application)
{
...
static void
pps_daemon_application_finalize (GObject *object)
{
        PpsDaemonApplication *application = PPS_DAEMON_APPLICATION (object);

        g_warn_if_fail (g_hash_table_size (application->pending_invocations) == 0);
        g_hash_table_destroy (application->pending_invocations);

        g_list_free_full (application->docs, (GDestroyNotify) pps_doc_free);

        G_OBJECT_CLASS (pps_daemon_application_parent_class)->finalize (object);
}

static void
pps_daemon_application_class_init (PpsDaemonApplicationClass *klass)
{
        GObjectClass *object_class = G_OBJECT_CLASS (klass);
        GApplicationClass *g_application_class = G_APPLICATION_CLASS (klass);

        object_class->finalize = pps_daemon_application_finalize;

        g_application_class->dbus_register = pps_daemon_application_dbus_register;
        g_application_class->dbus_unregister = pps_daemon_application_dbus_unregister;
}
...
main (gint argc, gchar **argv)
{
        ...
        g_set_prgname ("papers-daemon");

        application = g_object_new (PPS_TYPE_DAEMON_APPLICATION,
                                    "application-id", PPS_DBUS_DAEMON_NAME,
                                    "flags", flags,
                                    NULL);
