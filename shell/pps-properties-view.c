...
struct _PpsPropertiesViewClass {
	GtkBoxClass base_class;
};

G_DEFINE_TYPE (PpsPropertiesView, pps_properties_view, GTK_TYPE_BOX)

static void
pps_properties_view_dispose (GObject *object)
{
	PpsPropertiesView *properties = PPS_PROPERTIES_VIEW (object);

	g_clear_pointer (&properties->uri, g_free);

	G_OBJECT_CLASS (pps_properties_view_parent_class)->dispose (object);
}

static void
pps_properties_view_class_init (PpsPropertiesViewClass *properties_class)
{
	GObjectClass *g_object_class = G_OBJECT_CLASS (properties_class);

	g_object_class->dispose = pps_properties_view_dispose;
}
...
static void
set_property (PpsPropertiesView *properties,
	      GtkGrid          *grid,
	      Property          property,
	      const gchar      *text,
	      gint             *row)
...
void
pps_properties_view_set_info (PpsPropertiesView *properties, const PpsDocumentInfo *info)
{
	GtkWidget *grid;
	gchar     *text;
	gint       row = 0;
	...
	if (info->fields_mask & PPS_DOCUMENT_INFO_TITLE) {
	...
	if (info->fields_mask & PPS_DOCUMENT_INFO_FORMAT) {
	...
	if (info->fields_mask & PPS_DOCUMENT_INFO_LINEARIZED) {
		set_property (properties, GTK_GRID (grid), LINEARIZED_PROPERTY, info->linearized, &row);
	}
	if (info->fields_mask & PPS_DOCUMENT_INFO_SECURITY) {
...
static void
pps_properties_view_init (PpsPropertiesView *properties)
{
...
void
pps_properties_view_register_type (GTypeModule *module)
{
	pps_properties_view_get_type ();
}

GtkWidget *
pps_properties_view_new (PpsDocument *document)
{
	PpsPropertiesView *properties;

	properties = g_object_new (PPS_TYPE_PROPERTIES,
				   "orientation", GTK_ORIENTATION_VERTICAL,
				   NULL);
	properties->uri = g_uri_unescape_string (pps_document_get_uri (document), NULL);
	properties->file_size = pps_document_get_size (document);
