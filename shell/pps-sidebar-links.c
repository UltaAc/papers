...
struct _PpsSidebarLinksPrivate {
	GtkWidget *tree_view;
	GActionGroup *group;
...
static void update_page_callback 			(PpsSidebarLinks    *sidebar_links,
							 gint               old_page,
							 gint               current_page);
static void row_activated_callback 			(GtkTreeView *treeview,
		                                         ...
		                                         gpointer user_data);
static void pps_sidebar_links_set_links_model            (PpsSidebarLinks *links,
							 GtkTreeModel   *model);
static void job_finished_callback 			(PpsJobLinks     *job,
				    		         PpsSidebarLinks *sidebar_links);
static void pps_sidebar_links_set_current_page           (PpsSidebarLinks *sidebar_links,
							 gint            current_page);
static void sidebar_collapse_recursive                  (PpsSidebarLinks *sidebar_links);
static void pps_sidebar_links_page_iface_init 		(PpsSidebarPageInterface *iface);
static gboolean pps_sidebar_links_support_document	(PpsSidebarPage  *sidebar_page,
						         PpsDocument     *document);
static void pps_sidebar_links_set_model			(PpsSidebarPage   *sidebar_page,
							 PpsDocumentModel *model);

static guint signals[N_SIGNALS];

G_DEFINE_TYPE_EXTENDED (PpsSidebarLinks,
                        pps_sidebar_links,
                        GTK_TYPE_BOX,
                        0,
                        G_ADD_PRIVATE (PpsSidebarLinks)
                        G_IMPLEMENT_INTERFACE (PPS_TYPE_SIDEBAR_PAGE,
					       pps_sidebar_links_page_iface_init))


static void
pps_sidebar_links_set_property (GObject      *object,
			       guint         prop_id,
			       const GValue *value,
			       GParamSpec   *pspec)
{
	PpsSidebarLinks *pps_sidebar_links = PPS_SIDEBAR_LINKS (object);

	switch (prop_id)
	{
	case PROP_MODEL:
		pps_sidebar_links_set_links_model (pps_sidebar_links, g_value_get_object (value));
		break;
	case PROP_DOCUMENT_MODEL:
		pps_sidebar_links_set_model (PPS_SIDEBAR_PAGE (pps_sidebar_links),
...
static void
pps_sidebar_links_get_property (GObject    *object,
			       guint       prop_id,
			       GValue     *value,
			       GParamSpec *pspec)
{
	PpsSidebarLinks *pps_sidebar_links;

	pps_sidebar_links = PPS_SIDEBAR_LINKS (object);
...
static void
pps_sidebar_links_dispose (GObject *object)
{
	PpsSidebarLinks *sidebar = PPS_SIDEBAR_LINKS (object);
	...
	G_OBJECT_CLASS (pps_sidebar_links_parent_class)->dispose (object);
}

static void
pps_sidebar_links_map (GtkWidget *widget)
{
	PpsSidebarLinks *links;

	links = PPS_SIDEBAR_LINKS (widget);

	GTK_WIDGET_CLASS (pps_sidebar_links_parent_class)->map (widget);

	if (links->priv->model) {
		pps_sidebar_links_set_current_page (links,
						   pps_document_model_get_page (links->priv->doc_model));
	}
}

static void
pps_sidebar_links_set_action_enabled (PpsSidebarLinks *pps_sidebar_links,
				     const char     *name,
				     gboolean        enabled)
{
	GAction *action = g_action_map_lookup_action (
			G_ACTION_MAP (pps_sidebar_links->priv->group), name);
		            ...
		            PpsSidebarLinks     *pps_sidebar_links)
{
	...
	g_return_if_fail (pps_sidebar_links->priv->document != NULL);
	...
	if (gtk_tree_selection_get_selected (selection, &model, &iter)) {
		...
		g_signal_handler_block (pps_sidebar_links->priv->doc_model,
					pps_sidebar_links->priv->page_changed_id);
		g_signal_emit (pps_sidebar_links, signals[LINK_ACTIVATED], 0, link);
		g_signal_handler_unblock (pps_sidebar_links->priv->doc_model,
					  pps_sidebar_links->priv->page_changed_id);
...
pps_links_popup_cmd_print_section (GSimpleAction *action,
				  ...
				  gpointer       pps_sidebar_links)
{
	PpsSidebarLinks *sidebar = PPS_SIDEBAR_LINKS (pps_sidebar_links);
	PpsSidebarLinksPrivate *priv = PPS_SIDEBAR_LINKS (sidebar)->priv;
	GtkNative *window;
	...
	if (gtk_tree_selection_get_selected (selection, &model, &iter)) {
		...
		document_links = PPS_DOCUMENT_LINKS (priv->document);

		first_page = pps_document_links_get_link_page (document_links, link);
		if (first_page == -1) {
			g_object_unref (link);
			return;
		}
		...
		if (gtk_tree_model_iter_next (model, &iter)) {
			...
			if (link) {
				last_page = pps_document_links_get_link_page (document_links, link);;
				g_object_unref (link);
			}
		} else {
			last_page = pps_document_get_n_pages (priv->document);
		}
		...
		if (PPS_IS_WINDOW (window)) {
			pps_window_print_range (PPS_WINDOW (window), first_page, last_page);
		}
	}
}
			...
			PpsSidebarLinks *sidebar)
{
	...
	if (!is_list_is_set) {
		...
		if (model_is_plain_list (model)) {
			is_list = TRUE;
			pps_sidebar_links_set_action_enabled (sidebar, "collapse-all", FALSE);
			pps_sidebar_links_set_action_enabled (sidebar, "expand-all", FALSE);
			pps_sidebar_links_set_action_enabled (sidebar, "expand-element", FALSE);
		}
	}

	if (!selected_path) {
		pps_sidebar_links_set_action_enabled (sidebar, "print-section", FALSE);
		pps_sidebar_links_set_action_enabled (sidebar, "expand-element", FALSE);
	}
	...
	pps_sidebar_links_set_action_enabled (sidebar, "print-section", TRUE);
	...
	pps_sidebar_links_set_action_enabled (sidebar, "expand-element", expand_under_sensitive);
	...
	if (gtk_tree_path_get_depth (selected_path) == 1) {
		...
		pps_sidebar_links_set_action_enabled (sidebar, "collapse-all", !all_collapsed);
	} else
		pps_sidebar_links_set_action_enabled (sidebar, "collapse-all", TRUE);
}
		 ...
		 gpointer		 user_data)
{
	GdkEvent *event;
	GtkTreePath *path;
	PpsSidebarLinksPrivate *priv = PPS_SIDEBAR_LINKS (user_data)->priv;
	...
	if (gdk_button_event_get_button (event) == GDK_BUTTON_SECONDARY) {
			...
			check_menu_sensitivity (GTK_TREE_VIEW (priv->tree_view), path, PPS_SIDEBAR_LINKS (user_data));
...
remove_path_descendants (const gchar *metadata_index, GtkTreePath *path)
{
	...
	for (first_iteration = TRUE; tmp; tmp = g_strstr_len (haystack, -1, path_token)) {
		...
		ret = pps_str_replace (haystack, needle, "");
		if (!strcmp (ret, "|")) {
			g_free (ret);
			ret = g_strdup ("");
		}
...
remove_path (const gchar *metadata_index, GtkTreePath *path)
{
	...
	if (g_strstr_len (metadata_index, -1, path_token)) {
		ret = pps_str_replace (metadata_index, path_token, "|");
		if (!strcmp (ret, "|")) {
			g_free (ret);
			ret = g_strdup ("");
		}
		 ...
		 gpointer      data)
{
	...
	pps_sidebar_links = PPS_SIDEBAR_LINKS (data);
	priv = pps_sidebar_links->priv;

	window = gtk_widget_get_native (GTK_WIDGET (pps_sidebar_links));
	if (!PPS_IS_WINDOW (window)) {
		g_warning ("Could not find PpsWindow metadata, index_{expand,collapse} metadata won't be saved");
		return GDK_EVENT_PROPAGATE;
	}

	metadata = pps_window_get_metadata (PPS_WINDOW (window));
	if (metadata == NULL)
		return GDK_EVENT_PROPAGATE;

	index_collapse = NULL;
	/* If expanded row is in 'index_collapse' we remove it from there. */
	if (pps_metadata_get_string (metadata, "index-collapse", &index_collapse)) {
		new_index = remove_path (index_collapse, expanded_path);
		if (new_index) {
			pps_metadata_set_string (metadata, "index-collapse", new_index);
			g_free (new_index);
		}
	}
	...
	if (!expand) {
		...
		if (pps_metadata_get_string (metadata, "index-expand", &index_expand)) {
			...
			if (g_strstr_len (index_expand, -1, path_token) == NULL) {
				...
				pps_metadata_set_string (metadata, "index-expand", new_index);
				g_free (new_index);
			}
		} else
			pps_metadata_set_string (metadata, "index-expand", path_token);
		  ...
		  gpointer     data)
{
	...
	if (pps_metadata_get_string (metadata, "index-expand", &index_expand)) {
		...
		if (new_index) {
			...
			if (tmp) {
				pps_metadata_set_string (metadata, "index-expand", tmp);
				g_free (tmp);
			} else {
				pps_metadata_set_string (metadata, "index-expand", new_index);
			}
	...
	if (expand) {
		...
		if (pps_metadata_get_string (metadata, "index-collapse", &index_collapse)) {
			...
			pps_metadata_set_string (metadata, "index-collapse", path_token);
 ...
 * by pps_sidebar_links_set_current_page() function (which automatically
...
static void
sidebar_collapse_recursive (PpsSidebarLinks *sidebar_links)
{
	PpsSidebarLinksPrivate *priv;
	GtkNative *window;
	PpsMetadata *metadata;
	...
	if (PPS_IS_WINDOW (window)) {
		metadata = pps_window_get_metadata (PPS_WINDOW (window));
		if (metadata)
			pps_metadata_get_string (metadata, "index-expand", &index_expand);
	}
...
static void
pps_links_popup_cmd_collapse_all (GSimpleAction *action,
				 GVariant      *parameter,
				 gpointer       user_data)
{
	PpsSidebarLinks *sidebar_links = PPS_SIDEBAR_LINKS (user_data);
	PpsSidebarLinksPrivate *priv = sidebar_links->priv;
	GtkNative *window;
	PpsMetadata *metadata;
	...
	if (PPS_IS_WINDOW (window)) {
		metadata = pps_window_get_metadata (PPS_WINDOW (window));
		if (metadata) {
			pps_metadata_set_string (metadata, "index-expand", "");
	...
	if (index_collapse && strcmp (index_collapse->str, "|")) {
		pps_metadata_set_string (metadata, "index-collapse", index_collapse->str);
		g_string_free (index_collapse, TRUE);
	} else if (index_collapse) {
		pps_metadata_set_string (metadata, "index-collapse", "");
		g_string_free (index_collapse, TRUE);
	}
}
...
static void
pps_links_popup_cmd_expand_all (GSimpleAction *action,
			       GVariant      *parameter,
			       gpointer       user_data)
{
	PpsSidebarLinks *sidebar_links = PPS_SIDEBAR_LINKS (user_data);
	...
	if (index_expand && strcmp (index_expand->str, "|")) {
		pps_metadata_set_string (metadata, "index-expand", index_expand->str);
...
static void
pps_links_popup_cmd_expand_element (GSimpleAction *action,
				   GVariant      *parameter,
				   gpointer       user_data)
{
	PpsSidebarLinks *sidebar_links = PPS_SIDEBAR_LINKS (user_data);
	...
	if (PPS_IS_WINDOW (window)) {
		metadata = pps_window_get_metadata (PPS_WINDOW (window));
		if (metadata) {
			if (pps_metadata_get_string (metadata, "index-expand", &index_expand_chars))
...
create_links_action_group (PpsSidebarLinks *pps_sidebar_links) {
	const GActionEntry popup_entries[] = {
		...
		{ "expand-element", pps_links_popup_cmd_expand_element },
	};
...
static void
pps_sidebar_links_init (PpsSidebarLinks *pps_sidebar_links)
{
	PpsSidebarLinksPrivate *priv;
	GtkTreeModel *model;
	GtkTreeIter iter;
	...
	priv = pps_sidebar_links_get_instance_private (pps_sidebar_links);;
	pps_sidebar_links->priv = priv;

	gtk_widget_init_template (GTK_WIDGET (pps_sidebar_links));

	priv->group = create_links_action_group (pps_sidebar_links);
	...
	gtk_list_store_set (GTK_LIST_STORE (model), &iter,
			    PPS_DOCUMENT_LINKS_COLUMN_MARKUP, markup,
			    PPS_DOCUMENT_LINKS_COLUMN_EXPAND, FALSE,
			    PPS_DOCUMENT_LINKS_COLUMN_LINK, NULL,
			    -1);
	g_free (markup);
}

static void
pps_sidebar_links_class_init (PpsSidebarLinksClass *pps_sidebar_links_class)
{
	GObjectClass   *g_object_class;
	GtkWidgetClass *widget_class;

	g_object_class = G_OBJECT_CLASS (pps_sidebar_links_class);
	widget_class = GTK_WIDGET_CLASS (pps_sidebar_links_class);

	g_object_class->set_property = pps_sidebar_links_set_property;
	g_object_class->get_property = pps_sidebar_links_get_property;
	g_object_class->dispose = pps_sidebar_links_dispose;

	widget_class->map = pps_sidebar_links_map;

	gtk_widget_class_set_template_from_resource (widget_class,
			"/org/gnome/papers/ui/sidebar-links.ui");
	gtk_widget_class_bind_template_child_private (widget_class, PpsSidebarLinks, tree_view);
	gtk_widget_class_bind_template_child_private (widget_class, PpsSidebarLinks, popup);
	...
	signals[LINK_ACTIVATED] = g_signal_new ("link-activated",
			 G_TYPE_FROM_CLASS (g_object_class),
		         ...
		         G_STRUCT_OFFSET (PpsSidebarLinksClass, link_activated),
		         NULL, NULL,
...
GtkWidget *
pps_sidebar_links_new (void)
{
	return GTK_WIDGET (g_object_new (PPS_TYPE_SIDEBAR_LINKS, NULL));
}

typedef struct PpsSidebarLinkPageSearch {
	gint page;
	gint best_existing;
} PpsSidebarLinkPageSearch;

static gint
page_link_tree_search_best_page (gpointer page_ptr, PpsSidebarLinkPageSearch* data)
{
...
static void
pps_sidebar_links_set_current_page (PpsSidebarLinks *sidebar_links,
				   gint            current_page)
{
	GtkTreeSelection *selection;
	...
	PpsSidebarLinkPageSearch search_data;
...
static void
update_page_callback (PpsSidebarLinks *sidebar_links,
		      gint            old_page,
		      gint            new_page)
{
	pps_sidebar_links_set_current_page (sidebar_links, new_page);
}
...
update_page_link_tree_foreach (GtkTreeModel *model,
			       ...
			       gpointer      data)
{
	PpsSidebarLinks *sidebar_links = data;
	PpsSidebarLinksPrivate *priv = sidebar_links->priv;
	PpsDocumentLinks *document_links = PPS_DOCUMENT_LINKS (priv->document);
	PpsLink *link;
	int page;
	...
	page = pps_document_links_get_link_page (document_links, link);
	g_object_unref (link);
...
static void
pps_sidebar_links_set_links_model (PpsSidebarLinks *sidebar_links,
				  GtkTreeModel   *model)
{
	PpsSidebarLinksPrivate *priv = sidebar_links->priv;
...
static void
job_finished_callback (PpsJobLinks     *job,
		       PpsSidebarLinks *sidebar_links)
{
	PpsSidebarLinksPrivate *priv = sidebar_links->priv;
	GtkTreeSelection *selection;
	...
	pps_sidebar_links_set_links_model (sidebar_links, job->model);
	...
	if (PPS_IS_WINDOW (window)) {
		...
		if (metadata) {
			pps_metadata_get_string (metadata, "index-expand", &index_expand);
			pps_metadata_get_string (metadata, "index-collapse", &index_collapse);
		}
	}
	...
	pps_sidebar_links_set_current_page (sidebar_links,
					   pps_document_model_get_page (priv->doc_model));
}

static void
pps_sidebar_links_document_changed_cb (PpsDocumentModel *model,
				      GParamSpec      *pspec,
				      PpsSidebarLinks  *sidebar_links)
{
	...
	if (!pps_document_links_has_document_links (PPS_DOCUMENT_LINKS (document)))
		return;
	...
	priv->job = pps_job_links_new (document);
...
static void
pps_sidebar_links_set_model (PpsSidebarPage   *sidebar_page,
			    PpsDocumentModel *model)
{
	PpsSidebarLinks *sidebar_links = PPS_SIDEBAR_LINKS (sidebar_page);
	PpsSidebarLinksPrivate *priv = sidebar_links->priv;
	...
	g_signal_connect (model, "notify::document",
			  G_CALLBACK (pps_sidebar_links_document_changed_cb),
			  sidebar_page);
}

static gboolean
pps_sidebar_links_support_document (PpsSidebarPage  *sidebar_page,
				   PpsDocument *document)
{
	return (PPS_IS_DOCUMENT_LINKS (document) &&
		    pps_document_links_has_document_links (PPS_DOCUMENT_LINKS (document)));
}

static void
pps_sidebar_links_page_iface_init (PpsSidebarPageInterface *iface)
{
	iface->support_document = pps_sidebar_links_support_document;
	iface->set_model = pps_sidebar_links_set_model;
}
