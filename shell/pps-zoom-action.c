...
typedef struct {
        ...
        GtkPopover      *popup;
} PpsZoomActionPrivate;

G_DEFINE_TYPE_WITH_PRIVATE (PpsZoomAction, pps_zoom_action, GTK_TYPE_BOX)

#define GET_PRIVATE(o) pps_zoom_action_get_instance_private (o)
...
static void
pps_zoom_action_set_zoom_level (PpsZoomAction *zoom_action,
                               float         zoom)
{
	PpsZoomActionPrivate *priv = GET_PRIVATE (zoom_action);
        gchar *zoom_str;
        float  zoom_perc;
        guint  i;
...
static void
pps_zoom_action_update_zoom_level (PpsZoomAction *zoom_action)
{
	PpsZoomActionPrivate *priv = GET_PRIVATE (zoom_action);

        float       zoom = pps_document_model_get_scale (priv->model);

        zoom *= 72.0 / pps_document_misc_get_widget_dpi  (GTK_WIDGET (zoom_action));

        pps_zoom_action_set_zoom_level (zoom_action, zoom);
}

static void
zoom_changed_cb (PpsDocumentModel *model,
                 GParamSpec      *pspec,
                 PpsZoomAction    *zoom_action)
{
        pps_zoom_action_update_zoom_level (zoom_action);
}
                     ...
                     PpsZoomAction    *zoom_action)
{
        ...
        gtk_widget_set_sensitive (GTK_WIDGET (zoom_action), pps_document_get_n_pages (document) > 0);

        pps_zoom_action_update_zoom_level (zoom_action);
}

static void
pps_zoom_action_set_width_chars (PpsZoomAction *zoom_action,
                                gint          width)
{
	PpsZoomActionPrivate *priv = GET_PRIVATE (zoom_action);
...
static void
pps_zoom_action_populate_free_zoom_section (PpsZoomAction *zoom_action)
{
	PpsZoomActionPrivate *priv = GET_PRIVATE (zoom_action);
        gdouble max_scale;
        guint   i;
        ...
        pps_zoom_action_set_width_chars (zoom_action, width);
}

static void
max_zoom_changed_cb (PpsDocumentModel *model,
                     GParamSpec      *pspec,
                     PpsZoomAction    *zoom_action)
{
	PpsZoomActionPrivate *priv = GET_PRIVATE (zoom_action);
        ...
        pps_zoom_action_populate_free_zoom_section (zoom_action);
}
                    ...
                    PpsZoomAction *zoom_action)
{
	PpsZoomActionPrivate *priv = GET_PRIVATE (zoom_action);
        ...
        pps_document_model_set_scale (priv->model,
                                     zoom * pps_document_misc_get_widget_dpi (GTK_WIDGET (zoom_action)) / 72.0);
        g_signal_emit (zoom_action, signals[ACTIVATED], 0, NULL);
}
...
void
pps_zoom_action_set_model (PpsZoomAction *zoom_action,
			  PpsDocumentModel *model)
{
	PpsZoomActionPrivate *priv;

	g_return_if_fail (PPS_IS_ZOOM_ACTION (zoom_action));
...
static void
pps_zoom_action_finalize (GObject *object)
{
        PpsZoomAction *zoom_action = PPS_ZOOM_ACTION (object);
	PpsZoomActionPrivate *priv = GET_PRIVATE (zoom_action);
        ...
        G_OBJECT_CLASS (pps_zoom_action_parent_class)->finalize (object);
}

static void
setup_initial_entry_size (PpsZoomAction *zoom_action)
{
        gint width;
...
static void
pps_zoom_action_constructed (GObject *object)
{
        PpsZoomAction *zoom_action = PPS_ZOOM_ACTION (object);
	PpsZoomActionPrivate *priv = GET_PRIVATE (zoom_action);

        G_OBJECT_CLASS (pps_zoom_action_parent_class)->constructed (object);
...
static void
pps_zoom_action_class_init (PpsZoomActionClass *klass)
{
        GObjectClass *object_class = G_OBJECT_CLASS (klass);
        GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(klass);

        object_class->finalize = pps_zoom_action_finalize;
        object_class->constructed = pps_zoom_action_constructed;

        gtk_widget_class_set_template_from_resource (widget_class,
                        "/org/gnome/papers/ui/zoom-action.ui");
        gtk_widget_class_bind_template_child_private (widget_class,
                                                      PpsZoomAction,
                                                      entry);
	gtk_widget_class_bind_template_child_private (widget_class, PpsZoomAction, popup);
	gtk_widget_class_bind_template_child_private (widget_class, PpsZoomAction, menu);
	...
	gtk_widget_class_bind_template_callback (widget_class, pps_zoom_action_update_zoom_level);
...
static void
pps_zoom_action_init (PpsZoomAction *zoom_action)
{
